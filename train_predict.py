...
def train(train_loader,model,criterion_MSE,criterion_CE,optimizer,epoch,writer,root_path=None):
    ...
    for step,(sample_clip,recon_clip,step_label,recon_rate,motion_mask) in enumerate(train_loader):
...
def validation(val_loader,model,criterion_MSE,criterion_CE,optimizer,epoch):
    ...
    with torch.no_grad():
        for step, (sample_clip, recon_clip, step_label, recon_rate,motion_mask) in enumerate(val_loader):
            data_time.update(time.time() - end)
